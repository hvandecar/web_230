...
class dataTable {
  ...
  init(){
    ...
    //function that regenerates the table and all it's previous settings
    this.afterTblLoad();
    //function to generate the recent changes
  ...
  async updateTableValues() {
    try {
      let response = await fetch('json/issues-data.json', { method: 'GET', cache: 'no-cache' });
      let data = await response.json();
      let totalNewChanges = data.issues.length;
      //base string
      let str = '';
      //add in each lists item to the string
      
      for (let x = 0; x < totalNewChanges; x++) {
        str += '<tr data-priority="' + data.issues[x].priority.toLowerCase()
        str += '"><th scope="row">' + data.issues[x].id 
        str += '</th><td>' + data.issues[x].type 
        str += '</td><td><a href="' + data.issues[x].project + '-' + data.issues[x].id  + '.html">' + data.issues[x].desc + '</a></td><td>'
        str += data.issues[x].priority + '</td><td>' 
        str += data.issues[x].assigned + '</td></tr>';
      }
      //add it into html
      this.rowHolder.insertAdjacentHTML('afterbegin', str);
      this.rows = this.rowHolder.querySelectorAll('tr');
    }
    catch {
      console.log(error);
  ...
  async updateRecentChanges() {
    try {
      let response = await fetch('json/recent-changes.json', { method: 'GET', cache: 'no-cache' });
      let data = await response.json();
      let totalNewChanges = data.changes.length;
      //base string
      let str = '<ul>';

      //add in each lists item to the string
      for (let x = 0; x < totalNewChanges; x++) {
        str += '<li><a href="' + data.changes[x].id.toLowerCase() 
        str += '.html" title="' + data.changes[x].txt + '">' 
        str += data.changes[x].id + '</a> (' 
        str += data.changes[x].status + ')</li>';
      }
      str += '</ul>';
      //add it into html
